...
The RPM spec file provided here is for GRAM-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gram.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[GRAM git repository](https://github.com/gram/gram) but are not part of
...
Currently only `Source10` is used, for the example `gram.conf` file.
...
in the gram git repository. At present only an SVG version of the GRAM
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gram
...
needed when 0.12.1 is released, a proper fix is already in the GRAM git
master branch.
...
idea to build GRAM Core against a static version of that library compiled
...
The `gram.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gram-server` package. At this point, most RPM based
...
The `gram-server` package should have SELinux support. How to properly do
...
GRAM-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/gram/). He has been
packaging GRAM for Fedora at least since 2012.
...
4. I renamed the `gram` package that contains the Qt GUI to `gram-core` as
that appears to be how the general population refers to it, in contrast to
`gram-xt` or `gram-classic`. I wanted to make sure the general population
