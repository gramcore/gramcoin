...
###   gramcore.org and gram.org and compares them.
...
WORKINGDIR="/tmp/gram_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://gramcore.org"
HOST2="https://gram.org"
BASEDIR="/bin/"
VERSIONPREFIX="gram-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'gram-',
...
if [ $? -ne 0 ]; then
   echo "gram.org failed to provide signature file, but gramcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "gram.org and gramcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the GRAM Core binary release signing key installed?"
   fi
