...
other probitcoins. Reindexing using earlier versions will also not work
...
API, to create and destroy 'ephemeral' hidden services probitcoinmatically.
...
Then, to tunnel a SSL connection on 27566 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 27566 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ProxyPass http://127.0.0.1:7566/
        ProxyPassReverse http://127.0.0.1:7566/
...
- #6504 `b6fee6b` Rationalize currency unit to "Bitcoin" (Ross Nicoll)
